...
| `Worktree Navigator: Set Shared Files Sync Mode` | `manual / onCreate / onOpen / onCreateAndOpen / off` 모드 변경 |
...
| `worktreeNavigator.sharedFilesSyncMode` | `manual` | 공유 파일 동기화 모드 (`manual` / `onCreate` / `onOpen` / `onCreateAndOpen` / `off`) |
...
| `Worktree Navigator: Set Shared Files Sync Mode` | Change the shared-file sync mode (`manual / onCreate / onOpen / onCreateAndOpen / off`) |
...
| `worktreeNavigator.sharedFilesSyncMode` | `manual` | Shared-file sync mode (`manual`, `onCreate`, `onOpen`, `onCreateAndOpen`, or `off`) |
